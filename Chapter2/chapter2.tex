...
\begin{align}
    A_{ij} &\coloneqq K_{ij} - \frac{1}{3} \gamma_{ij} K, & \tr_{\gamma} A_{ij} &= \gamma^{ij} A_{ij} = 0.
\end{align}
...
\end{align}
The evolution equation of the spatial metric (\ref{eq:ADM_g_evol}) in conformal deposition formulation can hence be written as
\begin{flalign}
...
which is suggested naturally by equation(\ref{eq:con_g_evol_1}).
We define the same scaling factor as the conformal spatial metric (\ref{eq:conformal_metric_inv}) as
\begin{align}
...
The evolution equation of conformal spatial metric therefore becomes
\begin{align}
