...
In this chapter, we will introduction the Arnowitt-Deser-Misner (ADM) formulation, which is the one of the milestone of the 3+1 numerical relativity.
...
we omit the axuiliary variable $w^{ij}{}_k$ in order to avoid solving the gauge condition for $w^{ij}{}_k$ \ref{eq:FCF_conformal_w} and evolve the following hyperbolic equations for $\left(h^{ij}, A^{ij} \right)$
\begin{flalign}
