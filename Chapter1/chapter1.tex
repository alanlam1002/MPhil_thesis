...
Equation (\ref{eq:H_const_s4}) suffers from the mathematical nonuniqueness problems due to the positive exponent $(+5)$,
while the negative exponent $(-7)$ in equation (\ref{eq:H_const_s10}) guarantees the local uniquess of the solutions.
...
With the efficient elliptic multigrid solvers developped,
...
Each tensorial hyperbolic equation is reduced to two scalar equations under TT decomposition \cite{bonazzola2004constrained} and solved using spectral method,
...
we omit the axuiliary variable $w^{ij}{}_k$ in order to avoid solving the gauge condition of $w^{ij}{}_k$ \ref{eq:FCF_conformal_w} and evolve the following hyperbolic equations for $\left(h^{ij}, A^{ij} \right)$
\begin{flalign}
